...
void ntupler_PYTHIA_clean(double ptmin_trk=8,double ptmax_trk=300){ 
TH1D::SetDefaultSumw2();
 ...
 TFile *outf= new TFile(Form("ntuples_PYTHIA_mptonly_20140321/full_ntuple_%s_pt%d_%d_%s.root",infname.Data(),(int)ptmin_trk,(int)ptmax_trk,algo.Data()),"recreate");
 ...
 std::string trackVars="trackselect:eff:fake:trkfake:trkstatus:weight_unfold:pt:eta:phi:rmin:pt1:eta1:phi1:pt2:eta2:phi2:pt3:eta3:phi3:dphi:ptratio:jetspz:jetspt:jetseta:jetsrap:jetspz_12:jetspt_12:jetsrap_12:alpha:mpt_track:vz";
 ...
 TNtuple *nt_mpt_s_dR_ave=new TNtuple("nt_mpt_s_dR_ave","",mptVars.data());
 TNtuple *nt_mpt_b_dR_ave=new TNtuple("nt_mpt_b_dR_ave","",mptVars.data());
 TNtuple *nt_mpt_dR_ave=new TNtuple("nt_mpt_dR_ave","",mptVars.data());
 ...
 // for(int jentry=0;jentry<100;jentry++){
  ...
  float mpt_dR_tracks_uncorr_ave[30];
  float mpt_dR_ave[30];
  float mpt_dR_s_ave[30];
  float mpt_dR_b_ave[30];
  float mpt_dR_tracks_ave[30];
  ...
  for(int itrk=0;itrk<ftrk->nParticle;itrk++){
   ...
   float entry[]={pt,eta,phi,rmin,pNRec,smeared_pt,matchedpt,eff,trackselect,pt1,eta1,phi1,pt2,eta2,phi2,pt3,eta3,phi3,dphi,ptratio,jetspz,jetspt,jetseta,jetsrap,jetspz_12,jetspt_12,jetsrap_12,alpha,mpt_track,vz};
  ...
  for(int itrk=0;itrk<ftrk->nTrk;itrk++){
   ...
   if(njet>1){
    ...
    float entry[]={trackselect,eff,fake,trkfake,trkstatus,weight_unfold,pt,eta,phi,rmin,pt1,eta1,phi1,pt2,eta2,phi2,pt3,eta3,phi3,dphi,ptratio,jetspz,jetspt,jetseta,jetsrap,jetspz_12,jetspt_12,jetsrap_12,alpha,mpt_track,vz};
    nt_track->Fill(entry);
