...
void cumulative(int gensigbkgreco=3, int doMC=0, int index_var=2, bool doIntegrate=true,  int iAj=0, int doAve=1){
 ...
 for(int ipt=0;ipt<npt;ipt++){ 
  ...
  f[ipt]->cd();
  if(index_var==0) t[ipt]=(TTree*)f[ipt]->Get(Form("nt_%s%s%s",smpt[domp].Data(),skind[0].Data(),strave.Data()));
  else t[ipt]=(TTree*)f[ipt]->Get(Form("nt_%s%s_%s%s",smpt[domp].Data(),skind[0].Data(),svariable[index_var].Data(),strave.Data()));
  cout<<0<<endl;
  cout<<0.1<<endl;
 ...
 for(int icent=0;icent<ncent;icent++){
  ...
  cout<<1<<endl;
   ...
   if(ipt==npt-1){
    ...
    else  sintegrate[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[0].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
    ...
    else  sintegrate_stat[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[0].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
   }else{
    if(ialpha<nalpha/2 && ialpha>0) sintegrate[ialpha][ipt]=Form("-%s_%d+%s_%d",svariable[index_var].Data(),2*ialpha+1,svariable[index_var].Data(),2*(ialpha-1)+1);
    else if(ialpha==0)sintegrate[ialpha][ipt]=Form("-%s_%d",svariable[index_var].Data(),ialpha+1);
    else  sintegrate[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[0].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
   ...
   t_ref[ipt]->Draw(Form("%s>>h_ref_%d_%d_%d",sintegrate_ref[ialpha][ipt].Data(),ialpha,ipt,icent),Form("weight*cent_weight*(pthat>80 && pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6)&& ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15 && cent>=%d && cent<%d && abs(flavor1)<21)",jetpt1,jetpt2,etadijet,etadijet,Ajmin[iAj],Ajmax[iAj],cent_min[icent],cent_max[icent]),"");
  ...
  for(int ialpha=0;ialpha<nalpha/2+1;ialpha++ ){
   ...
   else t[ipt]->Draw(Form("%s>>h_%d_%d_%d",sintegrate[ialpha][ipt].Data(),ialpha,ipt,icent),Form("weight*cent_weight*(pthat>80 && pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6) && cent>=%d && cent<%d && ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15 && abs(flavor1)<21)",jetpt1,jetpt2,etadijet,etadijet, cent_min[icent],cent_max[icent],Ajmin[iAj],Ajmax[iAj]),"");
 ...
 if(doIntegrate){
  ...
  empty->SetMaximum(20); 
  empty->SetMinimum(-55); 
  empty2->SetMaximum(18); 
  empty2->SetMinimum(-15); 
 ...
 if(doIntegrate) empty->Draw();
  ...
  if(icent==1)drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.3,0.85);
   ...
   else drawText("reco-gen jet,lead Q",0.22,0.9);
 ...
 c1->SaveAs(Form("systematics_Q_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.png",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
 c1->SaveAs(Form("systematics_Q_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.pdf",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
