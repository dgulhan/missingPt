   ...
   for(Int_t i=0;i<columns;i++) {
      for(Int_t j=0;j<rows;j++) {
		 ...
		 if(i==0 && j==1){
		  ...
		  pad[i][j] = new TPad(padName.Data(),padName.Data(),Xlow[i],Ylow[j],Xup[i],Yup[j]);
		 }
		 else if(i==1 && j==1){
		  pad[i][j] = new TPad(padName.Data(),padName.Data(),Xlow[i],Ylow[j],Xup[i],Yup[j]);
		 }
...
void cumulative(int gensigbkgreco=3, int doMC=0, int index_var=2, bool doIntegrate=true,  int iAj=0, int doAve=1){
 ...
 for(int ipt=0;ipt<npt;ipt++){ 
  if(!doMC){  
   f[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_PbPb_20140406_v6/full_ntuple_hiForest_Jet80or95_GR_R_53_LV6_03Mar2014_1600CET_CMSSW_5_3_16_merged_pt%d_%d_akVs3Calo_v2.root",(int)ptmin[ipt],(int)ptmax[ipt])); 
 ...
 // cout<<"all events"<<endl;  
  ...
  TCanvas *c1 = new TCanvas("c1","",(ncent)*300,350);
  makeMultiPanelCanvas(c1,ncent,1,0.0,0.0,0.2,0.2,0.02);
 ...
 if(doIntegrate){
  ...
  empty2->SetMinimum(-10); 
   ...
   empty->GetYaxis()->SetTitleOffset(1.0);
   ...
   empty2->GetXaxis()->SetTitleOffset(1.0);
   ...
   empty2->GetYaxis()->SetTitleOffset(1.0);
 ...
 if(Ajmin[iAj]==0 && Ajmax[iAj]==1) drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f",etadijet),0.15,0.42);
 else if(Ajmin[iAj]==0) drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f, A_{J} < %.2f",etadijet,Ajmax[iAj]),0.15,0.42);
 else if(Ajmax[iAj]==1)drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f,  A_{J} > %.2f",etadijet,Ajmin[iAj]),0.15,0.42);
 else drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f, %.2f < A_{J} < %.0f",etadijet,Ajmin[iAj],Ajmax[iAj]),0.15,0.42);
 drawText(Form("p_{T,1}>%d, p_{T,2}>%d",jetpt1,jetpt2),0.35,0.9);
 drawText(Form("|#eta|<2, #Delta#phi_{1,2}>5#pi/6"),0.35,0.78);
 if(!doMC){
   drawText(Form("PbPb #sqrt{s_{NN}}=2.76 TeV 150/#mub"),0.15,0.66);
   drawText(Form("pp #sqrt{s_{NN}}=2.76 TeV 5.3/pb"),0.15,0.54);
 }
 if(doMC){
  if(!doGenJet)drawText(Form("akVs3Calo, ak3Calo"),0.15,0.34);
  else drawText(Form("gen jet"),0.15,0.34);
  drawText("#Delta#phi_{ave,ep}>1.6",0.35,0.24);
  drawText("v_{2}^{HF}>0.5",0.35,0.34);
  drawText(Form("%s",spart[gensigbkgreco].Data()),0.15,0.24);
 } 
 for(int icent=0;icent<ncent;icent++){ 
 c1->cd(ncent-icent);
 ...
 if(doIntegrate) empty->Draw();
  ...
  if(icent==1)drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.22,0.85);
 ...
 c1->SaveAs(Form("systematics/systematics_perpEP_lv2_cumulative_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.png",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
 c1->SaveAs(Form("systematics/systematics_perpEP_lv2_cumulative_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.pdf",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
