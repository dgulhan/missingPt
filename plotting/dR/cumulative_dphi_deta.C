...
void cumulative(int gensigbkgreco=3, int doMC=0, int index_var=2, bool doIntegrate=true,  int iAj=0, int doAve=1){
 ...
 for(int ipt=0;ipt<npt;ipt++){ 
  if(!doMC){  
   f[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_PbPb_20140411_v7//full_ntuple_hiForest_Jet80or95_GR_R_53_LV6_03Mar2014_1600CET_CMSSW_5_3_16_merged_pt%d_%d_akVs3Calo_v2.root",(int)ptmin[ipt],(int)ptmax[ipt])); 
   // f_ref[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_pp_20140414//full_ntuple_PP2013_HiForest_PromptReco_JsonPP_Jet80_PPReco_forestv82_pt%d_%d_ak3Calo.root",(int)ptmin[ipt],(int)ptmax[ipt])); 
      f_ref[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_PYTHIA_20140414//full_ntuple_pt50_pp2013_P01_prod22_v81_merged_forest_0_pt%d_%d_ak3Calo_doGenJet%d.root",(int)ptmin[ipt],(int)ptmax[ipt],doGenJet)); 

  }
  
  if(doMC){
   f[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_MC_mptonly_20140408_v6/full_ntuple_HydjetDrum_Pyquen_Dijet120_FOREST_Track8_Jet24_FixedPtHat_v0_pt%d_%d_akVs3Calo_doGenJet%d.root",(int)ptmin[ipt],(int)ptmax[ipt],doGenJet)); 
   f_ref[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_PYTHIA_20140414//full_ntuple_pt120_pp2013_P01_prod22_v81_merged_forest_0_pt%d_%d_ak3Calo_doGenJet%d.root",(int)ptmin[ipt],(int)ptmax[ipt],doGenJet)); 
  }
 ...
 cout<<Form("nt_%s%s_%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),svariable[index_var].Data(),strave.Data())<<endl; 
  ...
  cout<<0.3<<endl;
  hmpt_vs_alpha_ref[ipt]= new TH1D(Form("hmpt_ref_%d",ipt),"",nalpha/2,frac);
  if(ipt==npt-1)hmpt_vs_alpha_ref_cum= new TH1D(Form("hmpt_ref_cum_%d",ipt),"",nalpha/2,frac);
  ...
  cout<<1<<endl;
   if(ipt==npt-1){
    ...
    else if(ialpha==0)sintegrate[ialpha][ipt]=Form("-%s_%d",svariable[index_var].Data(),ialpha+1,ialpha);
    ...
    else  sintegrate[ialpha][ipt]=Form("-%s%s%s+%s_%d*(2*TMath::Pi()*(%d+0.1))",smpt[domp].Data(),skind[gensigbkgreco].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
   }
   ...
   mpt_ref[ipt][ialpha]=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h_ref[ipt][ialpha]->GetMean();
   mpterr_ref[ipt][ialpha]=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h_ref[ipt][ialpha]->GetMeanError();
   if(ipt==npt-1){
    h_ref_stat[ipt][ialpha]=new TH1D(Form("h_ref_stat_%d_%d",ialpha,ipt),"",100000,-100000,100000);
    ...
    mpt_ref_cum[ialpha]=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h_ref_stat[ipt][ialpha]->GetMean();
    mpterr_ref_cum[ialpha]=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h_ref[ipt][ialpha]->GetMeanError();
   }
   
   if(ipt<npt-1){
    cout<<"ipt "<<ipt<< " ialpha "<<ialpha<<" "<<mpt_ref[ipt][ialpha]<<endl;
  ...
  for(int icent=0;icent<ncent;icent++){
  hmpt_vs_alpha[ipt][icent]= new TH1D(Form("hmpt_%d_%d",ipt,icent),"",nalpha/2,frac);
  hmpt_vs_alpha_diff[ipt][icent]= new TH1D(Form("hmpt_diff_%d_%d",ipt,icent),"",nalpha/2,frac);
  if(ipt==npt-1){
   hmpt_vs_alpha_cum[icent]= new TH1D(Form("hmpt_ref_%d_%d",ipt,icent),"",nalpha/2,frac);
  } 
  for(int ialpha=0;ialpha<nalpha/2;ialpha++ ){
   ...
   else t[ipt]->Draw(Form("%s>>h_%d_%d_%d",sintegrate[ialpha][ipt].Data(),ialpha,ipt,icent),Form("cent_weight*(pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6) && cent>=%d && cent<%d && ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15)",jetpt1,jetpt2,etadijet,etadijet, cent_min[icent],cent_max[icent],Ajmin[iAj],Ajmax[iAj]),"");
   
   mpt=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h[ipt][ialpha][icent]->GetMean(); 
   mpterr=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h[ipt][ialpha][icent]->GetMeanError();
   ...
   if(ipt==npt-1){
    ...
    else t[ipt]->Draw(Form("%s>>h_stat_%d_%d_%d",sintegrate_stat[ialpha][ipt].Data(),ialpha,ipt,icent),Form("cent_weight*(pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6) && cent>=%d && cent<%d && ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15)",jetpt1,jetpt2,etadijet,etadijet, cent_min[icent],cent_max[icent],Ajmin[iAj],Ajmax[iAj]),""); 
    mpterr=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h[ipt][ialpha][icent]->GetMeanError();
    mpterr_cum=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h[ipt][ialpha][icent]->GetMeanError();
    mpt_cum=TMath::Pi()*(0.5)*((float)ialpha+0.5)*h_stat[ipt][ialpha][icent]->GetMean();
   }
 ...
 if(doIntegrate) empty->Draw();
  ...
  if(icent==1)drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.3,0.85);
   ...
   if(!doMC){
    if(icent==1)  drawText("PbPb-pp",0.3,0.9);
    else  drawText("PbPb-pp",0.22,0.9);
  ...
  if(index_var==3){
   for(int ialpha = 0 ;ialpha<hmpt_vs_alpha[npt-1][icent]->GetNbinsX();ialpha++){
    ...
    double x=hmpt_vs_alpha[npt-1][icent]->GetBinCenter(ialpha+1);
     ...
     TLine * l3= new TLine(x-0.035,y+yerr,x+0.035,y+yerr);
     TLine * l4= new TLine(x-0.035,y+yerr,x+0.035,y+yerr);
     TLine * l5= new TLine(x-0.035,y-yerr,x-0.035,y-yerr+min(0.25,yerr));
     TLine * l6= new TLine(x-0.035,y+yerr,x-0.035,y+yerr-min(0.25,yerr));
     TLine * l7= new TLine(x+0.035,y-yerr,x+0.035,y-yerr+min(0.25,yerr));
     TLine * l8= new TLine(x+0.035,y+yerr,x+0.035,y+yerr-min(0.25,yerr));
 ...
 c1->SaveAs(Form("cumulative2pir_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.png",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
 c1->SaveAs(Form("cumulative2pir_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.pdf",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
