...
void cumulative(int gensigbkgreco=3, int doMC=0, int index_var=2, bool doIntegrate=true,  int iAj=0, int doAve=1){
 ...
 for(int ipt=0;ipt<npt;ipt++){ 
  ...
  if(doMC){
   f[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_MC_mptonly_20140408_v6/full_ntuple_HydjetDrum_Pyquen_Dijet120_FOREST_Track8_Jet24_FixedPtHat_v0_pt%d_%d_akVs3Calo_doGenJet0.root",(int)ptmin[ipt],(int)ptmax[ipt],doGenJet)); 
   f_ref[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_MC_mptonly_20140408_v6/full_ntuple_HydjetDrum_Pyquen_Dijet120_FOREST_Track8_Jet24_FixedPtHat_v0_pt%d_%d_akVs3Calo_doGenJet0.root",(int)ptmin[ipt],(int)ptmax[ipt],doGenJet)); 
  }
 ...
 cout<<Form("nt_%s%s_%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),svariable[index_var].Data(),strave.Data())<<endl; 
  ...
  cout<<Form("nt_%s%s%s_wspec",smpt[domp].Data(),skind[gensigbkgreco].Data(),strave.Data())<<endl;
  if(index_var==0) t_ref[ipt]=(TTree*)f_ref[ipt]->Get(Form("nt_%s%s%s_wspec",smpt[domp].Data(),skind[3].Data(),strave.Data()));
  else t_ref[ipt]=(TTree*)f_ref[ipt]->Get(Form("nt_%s%s_%s%s_wspec",smpt[domp].Data(),skind[3].Data(),svariable[index_var].Data(),strave.Data()));
 ...
 for(int icent=0;icent<ncent;icent++){
  ...
  cout<<1<<endl;
   ...
   if(ipt==npt-1){
    ...
    else  sintegrate_ref[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[3].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
    ...
    else  sintegrate_stat_ref[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[3].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
   }else{
    if(ialpha<nalpha/2 && ialpha>0) sintegrate_ref[ialpha][ipt]=Form("-%s_%d+%s_%d",svariable[index_var].Data(),2*ialpha+1,svariable[index_var].Data(),2*(ialpha-1)+1);
    else if(ialpha==0)sintegrate_ref[ialpha][ipt]=Form("-%s_%d",svariable[index_var].Data(),ialpha+1);
    else  sintegrate_ref[ialpha][ipt]=Form("-%s%s%s+%s_%d",smpt[domp].Data(),skind[3].Data(),strave.Data(),svariable[index_var].Data(),nalpha-1);
 ...
 TH1D * empty=new TH1D("empty",Form(";%s;<#slash{p}_{T}^{#parallel}> (GeV)",axistitle[index_var].Data()),nalpha/2,frac);
 TH1D * empty2=new TH1D("empty2",Form(";%s;<#slash{p}_{T}^{#parallel}> (GeV)",axistitle[index_var].Data()),nalpha/2,frac);
 empty->Fill(0.5,1000); 
 ...
 if(doIntegrate) empty->Draw();
  ...
  if(icent==1)drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.3,0.85);
   ...
   else drawText("reweight - no reweight",0.22,0.9);
 ...
 empty->Draw();
  ...
  drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.22,0.85);
   ...
   }else drawText("PYTHIA+HYDJET,no weight",0.22,0.9);
 ...
 empty->Draw();
  ...
  drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.22,0.85);
   ...
   }else drawText("PYTHIA+HYDJET,weighted",0.22,0.9);

 } 
  
 c1->SaveAs(Form("systematics/systematics_wspec_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.png",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
 c1->SaveAs(Form("systematics/systematics_wspec_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.pdf",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin[iAj]*10),(int)(Ajmax[iAj]*10),ncent));
