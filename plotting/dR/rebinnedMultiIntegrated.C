...
void makeMultiPanelCanvas(TCanvas*& canv,
   ...
   for(Int_t i=0;i<columns;i++) {
      for(Int_t j=0;j<rows;j++) {
		 ...
		 if(i==0 && j==1){
          pad[i][j] = new TPad(padName.Data(),padName.Data(),Xlow[i],Ylow[j],Xup[i]*0.85,Yup[j]*0.82);
		  cout<<i<<" "<<j<<" "<<Xlow[i]<<" "<<Ylow[j]<<" "<<Xup[i]<<" "<<Yup[j]<<endl;
		 }
		 else if(i==0 && j==0){
		  pad[i][j] = new TPad(padName.Data(),padName.Data(),Xlow[i],Ylow[j]*0.8,Xup[i],Yup[j]);
		 }
		 else if(i==1 && j==1){
		  pad[i][j] = new TPad(padName.Data(),padName.Data(),0.82*Xlow[i],Ylow[j],Xup[i],Yup[j]);
		 }
         else pad[i][j] = new TPad(padName.Data(),padName.Data(),
            Xlow[i],Ylow[j],Xup[i],Yup[j]);
         if(i==0) pad[i][j]->SetLeftMargin(leftMargin);
		 else if(i==1 && j==1) pad[i][j]->SetLeftMargin(0.6*PadWidth);
		 ...
		 else if(i==0 && j==0) pad[i][j]->SetBottomMargin(0.45*PadHeight);
...
void multiIntegrated(int gensigbkgreco=3, int doMC=0, int index_var=2, bool doIntegrate=true,  double Ajmin=0, double Ajmax=1, int doAve=1){
 ...
 TTree *t_jet_genreco;  
 double mpt_genreco;
 double mpterr_genreco;  
 double mptsum_genreco[nalpha+1][ncent];
 double mptsumpos_genreco[nalpha+1][ncent];
 double mptsumneg_genreco[nalpha+1][ncent];
 TH1D* h_genreco[nalpha+1][ncent];
 TH1D *hsum_genreco[nalpha+1][ncent];
 TH1D * hmpt_vs_alpha_genreco[ncent];
 
 TFile *f_ref_genreco;
 TTree *t_ref_genreco;
 TTree *t_jet_ref_genreco; 
 double mpt_ref_genreco[nalpha+1];
 double mpterr_ref_genreco[nalpha+1];
 double mptsum_ref_genreco[nalpha+1];
 double mptsumpos_ref_genreco[nalpha+1];
 double mptsumneg_ref_genreco[nalpha+1];
 TH1D* h_ref_genreco[nalpha+1];
 TH1D *hsum_ref_genreco[nalpha+1];
 TH1D * hmpt_vs_alpha_ref_genreco;
 
 double mpt_diff_genreco;
 double mpterr_diff_genreco;
 double mptsum_diff_genreco[nalpha+1][ncent];
 double mptsumpos_diff_genreco[nalpha+1][ncent];
 double mptsumneg_diff_genreco[nalpha+1][ncent];
 TH1D *hsum_diff_genreco[nalpha+1][ncent];
 TH1D * hmpt_vs_alpha_diff_genreco[ncent];
 ...
 TString sintegrate_genreco[nalpha+1];
 TString s_genreco="";
 ...
 for(int ipt=0;ipt<npt;ipt++){ 
  ...
  if(doCompareGenReco && ipt==npt-1){
   f_genreco = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_MC_mptonly_20140325_pthat120/full_ntuple_HydjetDrum_Pyquen_Dijet120_FOREST_Track8_Jet24_FixedPtHat_v0_pt%d_%d_akVs3Calo_doGenJet%d.root",(int)ptmin[ipt],(int)ptmax[ipt],!doGenJet)); 
   f_ref_genreco = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_PYTHIA_mptonly_20140325_v5/full_ntuple_pt120_pp2013_P01_prod22_v81_merged_forest_0_pt%d_%d_ak3Calo_doGenJet%d.root",(int)ptmin[ipt],(int)ptmax[ipt],!doGenJet)); 
   t_jet_genreco=(TTree*)f_genreco->Get("nt_jet");
   t_jet_genreco=(TTree*)f_genreco->Get("nt_jet");
   
   if(index_var==0) t_genreco=(TTree*)f_genreco->Get(Form("nt_%s%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),strave.Data()));
   else t_genreco=(TTree*)f_genreco->Get(Form("nt_%s%s_%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),svariable[index_var].Data(),strave.Data()));
   
    for(int ialpha=0;ialpha<nalpha/2+1;ialpha++){
  cout<<1<<endl;
    if(doIntegrate){
    if(ialpha<nalpha/2)s_genreco=Form("-%s_%d",svariable[index_var].Data(),2*ialpha);
    if(ialpha<nalpha/2)sintegrate_genreco[ialpha]=s_genreco;
    else  sintegrate_genreco[ialpha]=Form("-%s%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),strave.Data());
   }else{
    if(ialpha<nalpha/2 && ialpha>0) sintegrate[ialpha][ipt]=Form("-%s_%d+%s_%d",svariable[index_var].Data(),2*ialpha,svariable[index_var].Data(),2*(ialpha-1));
   ...
   h_ref_genreco[ialpha]=new TH1D(Form("h_ref_%d_%d_genreco",ialpha,ipt),"",100000,-100000,100000);
   t_ref_genreco->Draw(Form("%s>>h_ref_%d_%d_genreco",sintegrate[ialpha][ipt].Data(),ialpha,ipt),Form("pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6)&& ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15",jetpt1,jetpt2,etadijet,etadijet,Ajmin,Ajmax),"");
   mpt_ref_genreco[ialpha]=h_ref[ipt][ialpha]->GetMean();
   mpterr_ref_genreco[ialpha]=h_ref[ipt][ialpha]->GetMeanError();
   

   hmpt_vs_alpha_ref_genreco->SetBinContent(ialpha+1,mpt_ref_genreco[ialpha]);
   hmpt_vs_alpha_ref_genreco->SetBinError(ialpha+1,mpterr_ref_genreco[ialpha]);

  }
  
    for(int icent=0;icent<ncent;icent++){
    hmpt_vs_alpha_genreco[icent]= new TH1D(Form("hmpt_%d_genreco",icent),"",nalpha/2+1,frac);
    hmpt_vs_alpha_diff_genreco[icent]= new TH1D(Form("hmpt_diff_%d_genreco",icent),"",nalpha/2+1,frac);

    for(int ialpha=0;ialpha<nalpha/2+1;ialpha++ ){
     h[ipt][ialpha][icent]=new TH1D(Form("h_%d_%d_%d",ialpha,ipt,icent),"",100000,-100000,100000);
     if(!doMC)t[ipt]->Draw(Form("%s>>h_%d_%d_%d",sintegrate[ialpha][ipt].Data(),ialpha,ipt,icent),Form("pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6) && cent>=%d && cent<%d && ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15",jetpt1,jetpt2,etadijet,etadijet, cent_min[icent],cent_max[icent],Ajmin,Ajmax),"");
     else t[ipt]->Draw(Form("%s>>h_%d_%d_%d",sintegrate[ialpha][ipt].Data(),ialpha,ipt,icent),Form("cent_weight*(pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6) && cent>=%d && cent<%d && ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15)",jetpt1,jetpt2,etadijet,etadijet, cent_min[icent],cent_max[icent],Ajmin,Ajmax),"");
     mpt_genreco=h_genreco[ialpha][icent]->GetMean(); 
     mpterr_genreco=h_genreco[ialpha][icent]->GetMeanError();
   
     mpt_diff_genreco=mpt_genreco-mpt_ref_genreco[ialpha];
     mpterr_diff_genreco=sqrt(pow(mpterr_genreco,2)+pow(mpterr_ref_genreco[ialpha],2));
   
     hmpt_vs_alpha_genreco[icent]->SetBinContent(ialpha+1,mpt_genreco);
     hmpt_vs_alpha_genreco[icent]->SetBinError(ialpha+1,mpterr_genreco);
   
     hmpt_vs_alpha_diff_genreco[icent]->SetBinContent(ialpha+1,mpt_diff_genreco);
     hmpt_vs_alpha_diff_genreco[icent]->SetBinError(ialpha+1,mpterr_diff_genreco);
 ...
 cout<<Form("nt_%s%s_%s%s",smpt[domp].Data(),skind[gensigbkgreco].Data(),svariable[index_var].Data(),strave.Data())<<endl; 
  ...
  cout<<1<<endl;
    if(doIntegrate){
    if(ialpha<nalpha/2)s[ipt]=Form("-%s_%d",svariable[index_var].Data(),2*ialpha);
   ...
   t_ref[ipt]->Draw(Form("%s>>h_ref_%d_%d",sintegrate[ialpha][ipt].Data(),ialpha,ipt),Form("pt1>%d && pt2>%d && abs(eta1)<%.1f && abs(eta2)<%.1f && dphi>(5*TMath::Pi()/6)&& ((pt1-pt2)/(pt2+pt1))>=%.2f && ((pt1-pt2)/(pt2+pt1))<%.2f && abs(vz)<15",jetpt1,jetpt2,etadijet,etadijet,Ajmin,Ajmax),"");
 ...
 if(Ajmin==0 && Ajmax==1) drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f",etadijet),0.05,0.42);
 else if(Ajmin==0) drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f, A_{J} < %.2f",etadijet,Ajmax),0.05,0.42);
 else if(Ajmax==1)drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f,  A_{J} > %.2f",etadijet,Ajmin),0.05,0.42);
 else drawText(Form("|#eta_{1}|,|#eta_{2}|<%.2f, %.2f < A_{J} < %.0f",etadijet,Ajmin,Ajmax),0.05,0.42);
 drawText(Form("p_{T,1}>%d, p_{T,2}>%d",jetpt1,jetpt2),0.05,0.9);
 drawText(Form("|#eta|<2, #Delta#phi_{1,2}>5#pi/6",jetpt1,jetpt2),0.05,0.78);
 if(!doMC){
 ...
 if(!doMC)drawText("CMS Preliminary",0.5,0.9);
 ...
 c1->cd((2*(ncent+1))-icent);
  ...
  drawText(Form("%d-%d %%",(int)(0.5*cent_min[icent]),(int)(0.5*cent_max[icent])),0.22,0.85);
   ...
   drawPatch(0.0,0.0,0.05,0.15);
   drawPatch(0.95,0.0,1,0.15);
 }
  
 c1->SaveAs(Form("rebinnedMultipanel_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.png",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin*10),(int)(Ajmax*10),ncent));
 c1->SaveAs(Form("rebinnedMultipanel_eta%d_doMC%d_%s_doIntegrate%d_doGenJet%d_kind%d_Aj%d_%d_ncent%d.pdf",(int)(etadijet*10),doMC,svariable[index_var].Data(),doIntegrate,doGenJet,gensigbkgreco,(int)(Ajmin*10),(int)(Ajmax*10),ncent));
 
 if(doGenJet && gensigbkgreco==0){
  TFile * outf = new TFile(Form("systematics/systematics_gen_gen_Aj%d_%d_kind%d.root",(int)(Ajmin*10),(int)(Ajmax*10),gensigbkgreco),"recreate");
  for(int icent=0;icent<ncent;icent++){
 ...
 if(!doGenJet && gensigbkgreco==3 && doMC){
  TFile * outf2 = new TFile(Form("systematics/systematics_reco_reco_Aj%d_%d_kind%d.root",(int)(Ajmin*10),(int)(Ajmax*10),gensigbkgreco),"recreate");
  for(int icent=0;icent<ncent;icent++){
   hmpt_vs_alpha_diff[npt-1][icent]->Write();
  }
  outf2->Close();
 ...
 if(doCompareGenReco){
  TH1D * hclone_ref;
  TH1D * hclone[ncent];
  TH1D * hclone_diff[ncent];
  hclone_ref = (TH1D*)hmpt_vs_alpha_ref_genreco->Clone(Form("hclone_ref"));
  hclone_ref->Divide(hmpt_vs_alpha_ref[npt-1]);

  TFile *outf3= new TFile(Form("systematics/systematics_reco_gen_Aj%d_%d_kind%d.root",(int)(Ajmin*10),(int)(Ajmax*10),gensigbkgreco),"recreate");
  for(int icent=0;icent<ncent;icent++){
   hclone[icent] = (TH1D*) hmpt_vs_alpha_genreco[icent]->Clone(Form("hclone%d",icent));
   hclone_diff[icent] = (TH1D*) hmpt_vs_alpha_diff_genreco[icent]->Clone(Form("hclone_diff%d",icent));
   hclone[icent]->Divide(hmpt_vs_alpha[npt-1][icent]);
   hclone_diff[icent]->Divide(hmpt_vs_alpha_diff[npt-1][icent]);
   hclone[icent]->Write();
   hclone_diff[icent]->Write();
  }
  hclone_ref->Write();
 ...
 // [7:09:13 AM] Yen-Jie Lee: for (int j=1;j<=pDiff->GetNbinsX();j++)
      // {
