...
void outcone(){
 ...
 for(int ipt=0;ipt<npt;ipt++){  
  ...
  if(doMC){
   f[ipt] = TFile::Open(Form("/afs/cern.ch/user/d/dgulhan/workDir/missingPt/ntuples_MC_20140310/full_ntuple_HiForest_Pythia_Hydjet_Jet80_Track8_Jet19_STARTHI53_LV1_merged_forest_0_pt%d_%d_akPu3Calo.root",(int)ptmin[ipt],(int)ptmax[ipt])); 
  }
  ...
  for(int iAj=0;iAj<nAj;iAj++ ){
   ...
   if(iAj<nAj-1)t[ipt]->Draw(Form("-%s%s+(alpha_0+alpha_1+alpha_2+alpha_3+alpha_4+alpha_5+alpha_6+alpha_7+alpha_8+alpha_9+alpha_10+alpha_11+alpha_12+alpha_13+alpha_14+alpha_15+alpha_16+alpha_17+alpha_18+alpha_19+alpha_20+alpha_21+alpha_22+alpha_23+alpha_24)>>h_%d_%d",smpt[domp].Data(),skind[gensigbkgreco].Data(),iAj,ipt),Form("pt1>%d && pt2>%d && abs(eta1)<%.2f && abs(eta2)<%.2f && dphi>(7*TMath::Pi()/8) && cent<%d && cent>=%d && ((pt1-pt2)/(pt1+pt2))>=%.2f && ((pt1-pt2)/(pt1+pt2))<%.2f",jetpt1,jetpt2,etadijet,etadijet,cent_max,cent_min,Aj[iAj],Aj[iAj+1]),"");
   else t[ipt]->Draw(Form("-%s%s+(alpha_0+alpha_1+alpha_2+alpha_3+alpha_4+alpha_5+alpha_6+alpha_7+alpha_8+alpha_9+alpha_10+alpha_11+alpha_12+alpha_13+alpha_14+alpha_15+alpha_16+alpha_17+alpha_18+alpha_19+alpha_20+alpha_21+alpha_22+alpha_23+alpha_24)>>h_%d_%d",smpt[domp].Data(),skind[gensigbkgreco].Data(),iAj,ipt),Form("pt1>%d && pt2>%d && abs(eta1)<%.2f && abs(eta2)<%.2f && dphi>(7*TMath::Pi()/8) && cent<%d && cent>=%d && ((pt1-pt2)/(pt1+pt2))>=%.2f",jetpt1,jetpt2,etadijet,etadijet,cent_max,cent_min,Aj[iAj]),"");
 ...
 if(!doMC){
  c1->SaveAs(Form("outcone_mpt_vs_Aj_%s%s_dijet%d_cent%d_%d.png",smpt[domp].Data(),skind[gensigbkgreco].Data(),(int)etadijet,cent_min,cent_max));
  c1->SaveAs(Form("outcone_mpt_vs_Aj_%s%s_dijet%d_cent%d_%d.pdf",smpt[domp].Data(),skind[gensigbkgreco].Data(),(int)etadijet,cent_min,cent_max));
 }
 if(doMC){
  c1->SaveAs(Form("outcone_mpt_vs_Aj_%s%s_dijet%d_cent%d_%d_MC.png",smpt[domp].Data(),skind[gensigbkgreco].Data(),(int)etadijet,cent_min,cent_max));
  c1->SaveAs(Form("outcone_mpt_vs_Aj_%s%s_dijet%d_cent%d_%d_MC.pdf",smpt[domp].Data(),skind[gensigbkgreco].Data(),(int)etadijet,cent_min,cent_max));
 }
}