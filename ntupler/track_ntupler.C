...
void track_ntupler(int nstep_cent=2,int nstep_accept=1,int nstep_pt=1,int nstep_rmin=1,double bin_pt_min=8,double bin_pt_max=100,int cent_min=0,int cent_max=200,int nevents=593463,bool dogenjet=false,TString algo="akPu3PF"){
 ...
 TString directory="/mnt/hadoop/cms/store/user/dgulhan/HIMC/Jet80/Track8_Jet17_STARTHI53_LV1/merged/";
 TString infname="HiForest_Pythia_Hydjet_Jet80_Track8_Jet17_STARTHI53_LV1_merged_forest_0";
 ...
 for(int icent=0; icent<nstep_cent;icent++){
   f_eff_cent[icent]= new TFile(Form("../stepwise_hists/eff_corr_nstep_cent%d_accept%d_pt%d_rmin%d_pt%d_%d_cent%d_%d.root",icent, icent, icent,icent,(int)bin_pt_min,(int)bin_pt_max,cent_min,cent_max));
 ...
 for(int iaccept=0;iaccept<nstep_accept;iaccept++){
   f_eff_accept[iaccept]= new TFile(Form("../stepwise_hists/eff_corr_nstep_cent%d_accept%d_pt%d_rmin%d_pt%d_%d_cent%d_%d.root",iaccept+1, iaccept, iaccept,iaccept,(int)bin_pt_min,(int)bin_pt_max,cent_min,cent_max));
 ...
 for(int ipt=0;ipt<nstep_pt;ipt++){
   f_eff_pt[ipt]= new TFile(Form("../stepwise_hists/eff_corr_nstep_cent%d_accept%d_pt%d_rmin%d_pt%d_%d_cent%d_%d.root",ipt+1, ipt+1, ipt,ipt,(int)bin_pt_min,(int)bin_pt_max,cent_min,cent_max));
 ...
 for(int irmin=0;irmin<nstep_rmin;irmin++){
   f_eff_rmin[irmin]= new TFile(Form("../stepwise_hists/eff_corr_nstep_cent%d_accept%d_pt%d_rmin%d_pt%d_%d_cent%d_%d.root",irmin+1, irmin+1, irmin+1,irmin,(int)bin_pt_min,(int)bin_pt_max,cent_min,cent_max));
 ...
 TFile *outf= new TFile(Form("../ntuples/track_ntuple_%s_%devts_cent_%d_accept_%d_pt_%d_rmin_%d_ptmin%d_ptmax%d_cent%d_%d_%s_dogenjet%d.root",infname.Data(),nevents,nstep_cent,nstep_accept,nstep_pt,nstep_rmin,(int)bin_pt_min,(int)bin_pt_max,cent_min,cent_max,algo.Data(),dogenjet),"recreate");
 ...
 // for(int jentry=0;jentry<nentries;jentry++){
  ...
  for(int itrk=0;itrk<ftrk->nParticle;itrk++){
   ...
   for(int irmin=0;irmin<nstep_rmin;irmin++){
    if(rmin<5)eff_rmin=eff_rmin*p_eff_rmin[irmin]->GetBinContent(p_eff_rmin[irmin]->GetXaxis()->FindBin(rmin));
   }
